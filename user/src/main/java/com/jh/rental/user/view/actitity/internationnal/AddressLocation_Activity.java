...
public class AddressLocation_Activity extends TitelBarAcitvity{
    TextView tv1,tv2;
...
private boolean  isData;
    private void initView() {
        ...
        if (getIntent().getStringExtra("addrsslocation")!=null){
            isData=true;
            value=getIntent().getStringExtra("addrsslocation");
        }else if (getIntent().getStringExtra("addrsslocations")!=null){
            isData=false;
            value=getIntent().getStringExtra("addrsslocations");
        }
        if (getIntent()!=null&&value!=null){
        ...
        public void onBindViewHolder(AddressLocationHolder holder, final int position) {
            ...
            holder.addresslv.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (isData){
                        OrderDetails.getOrderDetails().setCitysub(hotAddresses.get(position).getName());
                    }else {
                        OrderDetails.getOrderDetails().setDescity(hotAddresses.get(position).getName());
                    }
